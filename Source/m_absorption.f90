...
module m_absorption
    ...
    subroutine get_inelffs(tdsbr_t, max_int, kstep, ss, atf, nat, ak, relm, orthog, thmin, thmax)
        ...
        tp2 = 2.0_fp_kind * real(pi ** 2,kind=fp_kind)
        ...
        131 format(1h+,' Calculating absorptive scattering factor ', i4, ' of ', i4, '...' )
            ...
            do i_species = 1, nt
                ...
                tdsbr_t(ipa,i_species) = real(sum1,kind=fp_kind)
                ...
                if (all(g.eq.0.0_fp_kind)) then
                    mu_0 = mu_0 + real(sum1,kind=fp_kind)*nat(i_species)
                endif
        ...
        if(timing) then
            ...
            write(9834, '(a, g9.5, a, /)') 'Calculation of absorptive form factors took ', delta, 'seconds.'
            close(9834)
        endif
        ...
        ! M'_g = mu'_g/(4*pi)
        tdsbr_t = tdsbr_t / real(4d0 * pi,kind=fp_kind)
    ...
    function tds_calc_theta(theta)
        ...
        qz = ak * qzunit * real(ct,kind=fp_kind) - qkz
