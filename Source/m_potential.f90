...
970     write(*,*) ' Cannot access data file EELS_EDX_'//shell//'.dat'
     ...
     use global_variables
      ...
      integer(4) i,ii,iii,j,kval,nchoices,ZZ,nshells,norbitals
...
>>>>>>> GCC-compatibleMuSTEM
    ...
    function make_absorptive_grates(nopiy,nopix,n_slices) result(projected_potential)
        ...
        use m_multislice, only: nat_slice,tau_slice,ss_slice
        ...
        do j = 1, n_slices
            ...
            write(6,199) to_string(sum(nat_slice(:,j)))
    199     format(1x, 'Number of atoms in this slice: ', a, /)
            ...
            do m=1,nt
                ...
                projected_potential(:,:,j) = projected_potential(:,:,j)+potential_from_scattering_factors(effective_scat_fact,&
                                             tau_slice(:,m,:nat_layer,j),nat_layer,nopiy,nopix,high_accuracy)
            enddo
...
>>>>>>> GCC-compatibleMuSTEM
    ...
    function make_qep_grates(idum) result(projected_potential)
        ...
        use m_precision, only: fp_kind
        use global_variables, only: nopiy, nopix, nt, relm, tp, ak, atf, high_accuracy, pi, fz
        use m_multislice
        use m_string, only: to_string
        ...
        complex(fp_kind) :: projected_potential(nopiy,nopix,n_qep_grates,n_slices)
