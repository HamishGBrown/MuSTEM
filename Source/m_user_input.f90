...
module m_user_input
    ...
    function init_input()
        ...
        integer*4::init_input,io,reason,i,stat
        character(120) :: temp_string
        logical::exists
        line_no = 0
        init_input = 1
        ...
        elseif(to_lower(trim(adjustl(temp_string))) .eq. "record") then
            ...
            exists = .true.
            do while (exists)
            inquire(file=trim(adjustl(temp_string)),exist = exists)
            if(exists) then
                write(*,*) 'File "',trim(adjustl(temp_string)),'", listed in "user_input.txt"'," already exists, and you've"
                write(*,*) "attempted to record a file of the same name, please choose from the following options:"
                write(*,*)
                write(*,*) "<1> Overwrite ",trim(adjustl(temp_string))
                write(*,*) "<2> Record a new driving file with a different name"
                write(*,*) "<3> Play ",trim(adjustl(temp_string))
                write(*,*) '<4> Quit muSTEM'
                write(*,*)
                read(*,*) i
                if (i==1) then
                    open(unit=1234, iostat=stat, file=trim(adjustl(temp_string)), status='old')
                    if (stat == 0) close(1234, status='delete')
                elseif(i==2) then
                    write(*,*) 'Input new driving file name'
                    read(*,*) temp_string
                elseif(i==3) then
            ...
            enddo
            open(unit=in_file_number, file=trim(adjustl(temp_string)), status='new')
...
997     write(*,*) 'ERROR: USER_INPUT.TXT is missing '
        goto 999
        write(*,*) 'ERROR:',trim(temp_string),' does not exist '
