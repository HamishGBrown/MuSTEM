...
module m_Hn0
    ...
    300 format (' Enter Input File   file.orb')
        ...
        read(66,*) lorb
        write(*,100) et,lorb
    100 format(' Ionization Threshold Energy: ',g15.8,' eV',/,&
              &' Angular momentum of bound state: ',I3)
    ...
    100       if(allocated(state_vector)) deallocate(state_vector)
              ...
              if(lorb.gt.0) then
    101             write(6,*) 'Enter a value for an initial&
                               &ml quantum number (-l,l)'
                    call get_input('input ml quantum number',state_vector(1,1))
                    if(iabs(state_vector(1,1)).gt. iabs(lorb)) then
                          write(6,*) 'Error unphysical quantum &
                                    &number please reenter'
                          goto 101
                    endif
              ...
              write(6,*) 'Inital l:',lorb,' ml:',state_vector(1,1),'l_pr',&
                      &state_vector(1,2),'ml_pr',state_vector(1,3)
              write(6,*) '<1> Continue'
...
!-------------------------------------------------------------------------------
    subroutine fill_state_vector(order, l_input)
        ...
        !Quadrupole and higher transitions
        if(order.gt.1) then
            !limit sum over ml
              ...
              do while(ml.le.ml_max)
                     ...
                     !-----------------------------------------
                          ...
                          do while(ml_p.le.ml_p_max)
                                ...
                                ml_p=ml_p+1
                                if(j.gt.9999) then
                                  write(*,*) 'Too many states will crash now'
                                  stop
                                endif
                          enddo
                          l_p=l_p+1
    ...
    real(fp_kind) FUNCTION CLEB(A1,A2,A3,B2,B3)
        ...
        CLEB = 0.0_fp_kind
        B1 = B3 - B2
        IF ( ABS ( B1 ) .GT. A1 .OR. ABS ( B2 ) .GT. A2 &
                               &.OR. ABS ( B3 ) .GT. A3      ) RETURN
        TEST1 = A1 + B1 + 100.0 - float(INT ( A1 + B1 + 100.0 ))
        TEST2 = A2 + B2 + 100.0 - float(INT ( A2 + B2 + 100.0 ))
        TEST3 = A1 + A2 + A3    - float(INT ( A1 + A2 + A3    ))
        IF ( ( TEST1 .GT. 0.1_fp_kind ) .OR. ( TEST2 .GT. 0.1_fp_kind) &
                                        .OR. ( TEST3 .GT. 0.1_fp_kind)) RETURN
      ...
      2 SU = 0.0_fp_kind
        ...
        DO K = M, MO
           ...
           J4 = K4 - K
           J5 = K5 - K
           SU = -SU + 1. / ( FAK(K) * FAK(J1) * FAK(J2) &
                          &* FAK(J3)* FAK(J4) * FAK(J5) )
        END DO
        J6 = K2 + K3 - 1
        J7 = K2 + K4 - 1
        J8 = K1 + K5 - 1
        ...
        CLEB = SS_cleb * SU * SQRT ( DEL * FAK(K6+1) * FAK(K4-1) * FAK(K5-1)* &
             &FAK(J6) * FAK(J7) * FAK(J8) * ( 2.0_fp_kind * A3 + 1.0_fp_kind ) )
    ...
    real(fp_kind) function plgndr(l_temp,m,x)
        ...
        if(m.lt.0.or.m.gt.l_temp.or.abs(x).gt.1.0_fp_kind) then
          write(*,*) ' ERROR: Bad arguments in plgndr'
          stop
        endif
...
!--------------------------------------------------------------------------------
    ...
    SAVE cof,stp
    DATA cof,stp/76.18009172947146_fp_kind,-86.50532032941677_fp_kind,&
                &24.01409824083091_fp_kind,-1.231739572450155_fp_kind,&
                &.1208650973866179e-2_fp_kind,-.5395239384953e-5_fp_kind,&
                &2.5066282746310005_fp_kind/
    x=xx
    y=x
    tmp=x+5.5_fp_kind
...
11  continue
    ...
    subroutine calc_gsplint(ml,lpr,mlpr,qpos_local,qmin_local&
                          &,gint_local,dgint_local)
...
140 eloss = eval + et
    ...
    ! calculate integral
    if(lorb.eq.0) then
          gint_local(lpr,iqstp,0)=h*(4.0_fp_kind*sumod(0)+&
                                    &2.0_fp_kind*sumev(0))/3.0_fp_kind
    endif

    if(lorb.eq.1) then
          gint_local(lpr,iqstp,-1)=h*(4.0_fp_kind*sumod(-1)+&
                                     &2.0_fp_kind*sumev(-1))/3.0_fp_kind
          gint_local(lpr,iqstp,1)=h*(4.0_fp_kind*sumod(1)+&
                                     2.0_fp_kind*sumev(1))/3.0_fp_kind
    endif
...
30  R=h         !radius at I=1
    ...
    do I=2,NSTEP-1   !calculation of Pel(I+1)
          ...
          if(fp_kind.eq.Double.and.abs(Pel(I+1)).gt.dmax_val.or&
           &.fp_kind.eq.Single.and.abs(Pel(I+1)).gt.smax_val) then
              do J=1,I+1
    ...
    DPel=(2.0_fp_kind*Pel(NSTEPX-4)-16.0_fp_kind*Pel(NSTEPX-3)+&
        &16.0_fp_kind*Pel(NSTEPX-1)-2.0_fp_kind*Pel(NSTEPX))/(24.0_fp_kind*h)
...
21        format(' nrad is larger than maximum allowed!',/,'  nrad will be reset to ',I5)
          write(*,*) 'press enter to continue or Ctrl+C to exit'
          read(*,*)
          nrad=nradmax
    endif
    ...
    do i=nrad,1,-1
          ...
          sbtmp(3)=(15.0d0/(x*X*x*x)-6.0d0/(x*x))*dsin(x)+&
                  &(1.0d0/x-15.0d0/(x*x*x))*dcos(x)
          sbtmp(4)=(1.05d2/(x*x*x*x*x)-45.0d0/(x*x*x)+1.0d0/x)*dsin(x)+&
                  &(10.0d0/(x*x)-1.05d2/(x*x*x*x))*dcos(x)
    ...
    do i=2,n-1
          ...
          u(i)=(6.0_fp_kind*((gint_local(lpr,i+1,ilam)-&
                             &gint_local(lpr,i,ilam))/(x(i+1)-x(i))-&
                             &(gint_local(lpr,i,ilam)-gint_local(lpr,i-1,ilam))&
    ...
    if (ypn.gt..99e30_fp_kind) then
          ...
          qn=0.5_fp_kind
          un=(3.0_fp_kind/(x(n)-x(n-1)))*(ypn-(gint_local(lpr,n,ilam)-&
                                    &gint_local(lpr,n-1,ilam))/(x(n)-x(n-1)))
    endif
    dgint_local(lpr,n,ilam)=(un-qn*u(n-1))/(qn*dgint_local(lpr,n-1,ilam)+1.)
    do k=n-1,1,-1
          dgint_local(lpr,k,ilam)=dgint_local(lpr,k,ilam)*&
                                 &dgint_local(lpr,k+1,ilam)+u(k)
    enddo
    continue
    return
    END subroutine
...
1   if (khi-klo.gt.1) then
    ...
    h=xa(khi)-xa(klo)
    if (h.eq.0.0_fp_kind) then
      write(*,*) 'bad xa input in splint'
      read(*,*)
    endif
    a=(xa(khi)-x)/h
    b=(x-xa(klo))/h
    y=a*gint_local(lpr,klo,ilam)+b*gint_local(lpr,khi,ilam)+((a*a*a-a)&
      &*dgint_local(lpr,klo,ilam)+(b*b*b-b)*dgint_local(lpr,khi,ilam))*(h*h)/6.
    return
    END subroutine
...
!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
    ...
    DATA ZERO,ONE,TWO,TEN2,ABORT /0.0_fp_kind, 1.0_fp_kind, 2.0_fp_kind, &
                             &1.0_fp_kind, 2.0e4_fp_kind/
...
2000  FORMAT(' FOR XX = ',1PD12.3,' TRY SMALL-X  SOLUTIONS',' OR X NEGATIVE'/ ,&
...
2040  FORMAT(' XLMAX - XLMIN = DELL NOT AN INTEGER ',1P3D20.10/)
    ...
    DATA   ZERO,HALF,ONE,SIX,TEN/ 0.0E0_fp_kind, 0.5E0_fp_kind, &
                      &1.0E0_fp_kind, 6.0E0_fp_kind, 10.0E0_fp_kind /
    DATA  DZERO, RL35, ALOGE  /0.0E0_fp_kind, 35.0E0_fp_kind, &
                                 &0.4342944819E0_fp_kind /
    X     = XX
    ETA   = ETA1
    GH2   = X*(ETA + ETA - X)
    ...
    do ny=1,nopiy
          ...
          do nx=1,nopix
                ...
                do ilam = lammin,lammax
                      ...
                      m_l_temp = float(m_l)
                      call gsplint(qpos_local,nsplpts,qtmp1,tgint,lpr,&
                                   &ilamp,gint_local,dgint_local)
                      threej = cleb(lpr_temp,ilam_temp,l_temp,0.0_fp_kind,&
                                  &0.0_fp_kind)* (-1.0_fp_kind)**(lpr-ilam)/&
                                  &sqrt(2.0_fp_kind*float(lorb)+1.0_fp_kind)
                      cval = cval * threej * tgint
                      threej = cleb(lpr_temp,ilam_temp,l_temp,-m_lam_temp,-m_l_temp)&
                              &*(-1.0_fp_kind)**(-m_l+lpr-ilam) &
                              &/ sqrt(2.0_fp_kind*float(lorb)+1.0_fp_kind)
                ...
                enddo
                fq = ((-1)**m_lpr)*((-1)**m_lam)
                fq = fq*sqrt( 2.0_fp_kind*tp*float(2*lpr+1)*float(2*lorb+1) )
                fq = fq* csum
  ...
  100     format(a1,' State:',i3,'/',i3,' l',i3,' ml',i3,' lpr',i3,' mlpr',i3)
	...
	allocate(ion_tau(2,sum(natoms_slice_total)))
      do j=1,n_slices
        counter = 0
        !loop over ionization species in unit cell
        do atom=1,nat_slice_unitcell(target_atom,j)
          !if plane wave we only need to shift over the unit cell
          if(pw_illum) then
	...
	ion_tau(:,counter) = [x_coord,y_coord]
            !this is shifting fractionally on the supercell
            call make_shift_oned(Hn0_shiftx_coord(:,counter,j),nopix,x_coord)
            !this is shifting fractionally on the supercell
            call make_shift_oned(Hn0_shifty_coord(:,counter,j),nopiy,y_coord)
          !if focussed probe we need to shift over supercell
          elseif(all_atoms) then
            !tile the unit cell atom over the supercell
            do ss_atomx=1,ifactorx
              !tile the unit cell atom over the supercell
              do ss_atomy=1,ifactory
                x_coord = (tau_slice_unitcell(1,target_atom,atom,j)+&
                            &(float(ss_atomx)-1.0_fp_kind))/float(ifactorx)
                y_coord = (tau_slice_unitcell(2,target_atom,atom,j)+&
                           &(float(ss_atomy)-1.0_fp_kind))/float(ifactory)
                counter = counter+1
	ion_tau(:,counter) = [x_coord,y_coord]
                !this is shifting fractionally on the supercell
                call make_shift_oned(Hn0_shiftx_coord(:,counter,j),nopix,x_coord)
            ...
            !tile the unit cell atom over the supercell
            do ss_atomx=-xcells/2,xcells/2
              !tile the unit cell atom over the supercell
              do ss_atomy=-ycells/2,ycells/2
                x_coord = (tau_slice_unitcell(1,target_atom,atom,j)+&
                         &(float(ifactorx+ss_atomx)))/float(ifactorx)
                if (ss_atomx.ge.0) x_coord = (tau_slice_unitcell(1,target_atom,atom,j)&
                                           &+(float(ss_atomx)))/float(ifactorx)
                y_coord = (tau_slice_unitcell(2,target_atom,atom,j)+&
                            &(float(ifactorx+ss_atomy)))/float(ifactorx)
                if (ss_atomy.ge.0) y_coord = (tau_slice_unitcell(2,target_atom,atom,j)+&
                                        &(float(ss_atomy)))/float(ifactory)
                counter = counter+1
	ion_tau(:,counter) = [x_coord,y_coord]
                !this is shifting fractionally on the supercell
                call make_shift_oned(Hn0_shiftx_coord(:,counter,j),nopix,x_coord)
    ...
    function fourier_shift_array(arrayin,yshift,xshift) result(arrayout)
        ...
        call inplace_ifft(y,x,array)
        arrayout = real(array,kind = fp_kind)/nopiy/nopix

    end function

	subroutine choose_individual_targets(cells_out,targets,n_slices,n_cells,&
          &natoms_slice_total,ifactorx,ifactory,ion_tau,sumnatoms_slice_total)
		use output, only:nlines, array_from_txt_file

        integer*4::sumnatoms_slice_total
		logical::targets(sumnatoms_slice_total)
		integer*4::cells_out(n_cells),n_slices,n_cells
    integer*4::natoms_slice_total(n_slices,ifactorx,ifactory),ifactorx,ifactory
		real(fp_kind),allocatable::ion_tau(:,:)
		...
		if(ifile_exists) then
			...
			cells_out = 0
			cells_out(:numlines) = nint(array_from_txt_file('ionization_cells.txt'&
                                                                  &,numlines))
		else
			cells_out = 1
		end if
		...
		do k=1,size(targets)
			if(targets(k)) write(6,800) k,char(10), mod(ion_tau(1,k)*ifactorx,1.0), &
                                             &mod(ion_tau(2,k)*ifactory,1.0), &
                                  &floor(ion_tau(1,k)*ifactorx)+1,ifactorx, &
                                  &floor(ion_tau(2,k)*ifactory)+1,ifactory
		enddo
		write(*,*)
800     format('Target no. = ',I3,':',a1,'  x = ',f5.2,'  y = ',f5.2,&
            &'  unit cell x: ', i2,'/',i2,'  unit cell y: ', i2,'/',i2)
    end subroutine
