...
subroutine absorptive_stem(STEM,ionization,PACBED)
 ...
 if(double_channeling) then
    ...
    do i=1,nstates
      ...
      call binary_out(nopiy,nopix,quad_shift(ifft(nopiy,nopix,psi),nopiy,nopix),'Hn0_ml='//to_string(state_vector(i,1))//&
    ...
    allocate(Hn0_eels_detector(nopiy,nopix,numeels))
    do l=1,numeels
      Hn0_eels_detector(:,:,l) = make_detector(nopiy,nopix,ifactory,ifactorx,ss&
                                                     &,0.0_fp_kind,outerrad(l))
    enddo
 ...
 if(stem) then
    ...
    do i=1,ndet/nseg
    do j=1,nseg
      if(nseg>1) temp = make_detector(nopiy,nopix,ifactory,ifactorx,ss,inner(i)&
                                  ,outer(i),2*pi*j/nseg-seg_det_offset,2*pi/nseg)
      if(nseg==1) temp = make_detector(nopiy,nopix,ifactory,ifactorx,ss&
                                                    &,inner(i),outer(i))
      masks(:,:,(i-1)*nseg+j) = temp
    enddo
    enddo
    if (adf) then
...
902     format(1h+,1x,' df:',i3,'/',i3,' y:',i3,'/',i3,' x:',i3,'/',i3,' tilt:',i3,'/',i3'  Intensity:', f6.3)
            ...
            do k = 1, nstates
              ...
              do ii = i, n_cells
                ...
                if (any(ii==ncells)) then
                  ...
                  enddo;endif;
                endif !End output if statement
              enddo !end loop over cells in double channeling
            enddo !End loop over states
          enddo !End loop over targets
        endif !End double_channeling section
  ...
  do i = 1,maxval(ncells);do j = 1, n_slices
    ...
    if(fourDSTEM)  then
      call output_TEM_result(output_prefix,temp,'Diffraction_pattern',nopiy,nopix&
                          &,manyz,many_df,manytilt,zarray(z_indx(1)),probe_df(i_df)&
                          &,length,lengthdf,tilt_description(claue(:,ntilt),ak1,ss,ig1,ig2),&
 ...
 if(istem) then
  do i=1,nz;do l=1,imaging_ndf
   ...
   temp = tile_out_image(temp/nysample/nxsample,ifactory,ifactorx)
   call output_TEM_result(output_prefix,temp,'ISTEM',nopiy,nopix,manyz,&
                        &imaging_ndf>1,manytilt,z=zarray(i),lengthz=length,&
                        &lengthdf=lengthimdf,tiltstring = &
