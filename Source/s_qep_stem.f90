...
subroutine qep_stem(STEM,ionization,PACBED)
    ...
    !random variables
    integer(4) :: idum,z_indx(1)=-1
    !real(fp_kind) :: ran1
    complex(fp_kind) :: prop(nopiy,nopix,n_slices)
    ...
    !output
    real(fp_kind),dimension(nopiy,nopix) :: temp
...
! Precalculate the scattering factors on a grid
    ...
    forward_plan = fft_plan(nopiy,nopix,psi,-1,1)
    inverse_plan = fft_plan(nopiy,nopix,psi,+1,1)
    ...
    do ntilt=1,n_tilts_total
        ...
        do i = 1, n_slices
            call make_propagator(nopiy,nopix,prop(:,:,i),prop_distance(i),Kz(1),ss,claue(:,1),ifactorx,ifactory)
            prop(:,:,i) = prop(:,:,i) * bwl_mat
            !stop
            qep_grates(:,:,:,i) = exp(ci*pi*a0_slice(3,i)/Kz(ntilt)*projected_potential(:,:,:,i))/nopiy/nopix
            do j=1,n_qep_grates
            call inplace_fft(nopiy,nopix,qep_grates(:,:,j,i),norm=.true.)
            if(qep_mode.eq.3) qep_grates(:,:,j,i)= qep_grates(:,:,j,i)*bwl_mat
            if(qep_mode.ne.3) qep_grates(:,:,j,i) = ifft(nopiy,nopix,qep_grates(:,:,j,i)*bwl_mat,norm=.true.)
            enddo
        enddo
...
!#endif
        ...
        psi_elastic=0_fp_kind
            ...
            do i = 1,maxval(ncells)
                do j = 1, n_slices
                  ...
                  call qep_multislice_iteration(psi,prop(:,:,j),qep_grates(:,:,:,j),nopiy,nopix,ifactory,ifactorx,idum,&
        ...
        do iz=1,nz
            ...
            ! Integrate the elastic diffraction pattern
            psi_out = fft(nopiy,nopix,psi_elastic(:,:,iz))
            if(stem) stem_image(ny,nx,i_df,1:ndet,iz) = sum(sum(spread(cbed(:,:,iz),dim=3,ncopies=ndet)*masks(:,:,:),dim=1),dim=1)
            if(stem) stem_elastic_image(ny,nx,i_df,1:ndet,iz)= sum(sum(spread(abs(psi_out)**2,dim=3,ncopies =ndet)&
            ...
            if(pacbed) then
                ...
                if (output_thermal.and.fourDSTEM) then
                        ...
                        if (probe_ndf>1) filename = trim(adjustl(filename))//defocus_string(probe_df(i_df),lengthdf)
                        if (nz>1) filename = trim(adjustl(filename))//'_z='//to_string(int(zarray(iz)))//'_A'
                        filename = trim(adjustl(filename))//'_pp_'//to_string(nx)//'_'//to_string(ny)&
                                   //'_Elastic_Diffraction_pattern'
