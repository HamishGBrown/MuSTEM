...
subroutine qep_tem
    ...
    if(pw_illum) then
        do i=1,imaging_ndf
            ctf(:,:,i) =  make_ctf([0.0_fp_kind,0.0_fp_kind,0.0_fp_kind],imaging_df(i),imaging_cutoff,imaging_aberrations&
                                                                                                     ,imaging_apodisation)
        enddo
#ifdef GPU
    ...
    do i = 1, n_slices
        call make_propagator(nopiy,nopix,prop(:,:,i),prop_distance(i),Kz(ntilt),ss,claue(:,ntilt),ifactorx,ifactory)
        ...
        do j=1,n_qep_grates
        psi = fft(nopiy,nopix,qep_grates(:,:,j,i),norm=.true.)
        if(qep_mode == 3) qep_grates(:,:,j,i)= psi*bwl_mat
        if(qep_mode .ne. 3) qep_grates(:,:,j,i) = ifft(nopiy,nopix,psi*bwl_mat,norm=.true.)
        enddo
        endif
    enddo
        ...
        do i_cell = 1,maxval(ncells)
            do i_slice = 1, n_slices
                ...
                nran = floor(n_qep_grates*ran1(idum)) + 1
                  ...
                  call qep_multislice_iteration(psi,prop(:,:,i_slice),qep_grates(:,:,:,i_slice),nopiy,nopix,ifactory,ifactorx,&
                                                                        idum,n_qep_grates,qep_mode,shift_arrayy,shift_arrayx)
            ...
            enddo ! End loop over slices
                ...
                if (any(i_cell==ncells)) then
                    ...
                    cbed(:,:,z_indx(1)) = cbed(:,:,z_indx(1)) + abs(psi_out)**2

                    ! Accumulate image
                    if(pw_illum) then
                    do i=1,imaging_ndf
                        temp= abs(ifft(nopiy,nopix,ctf(:,:,i)*psi_out,norm=.true.))**2
                        tem_image(:,:,z_indx(1),i) = tem_image(:,:,z_indx(1),i)+temp
                    enddo
                    endif
...
900     format(1h+,  'QEP pass:', i4, '/', i4, ' Intensity: ', f8.3)
    ...
    do i=1,nz
        ...
        do j=1,imaging_ndf
            ! Elastic image
            psi = fft(nopiy, nopix, psi_elastic(:,:,i),norm=.true.)* ctf(:,:,j)
