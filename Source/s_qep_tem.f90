...
subroutine qep_tem
    ...
    use m_precision;use m_numerical_tools;use global_variables;use m_lens;use FFTW3
#ifdef GPU
  ...
  complex(fp_kind),device,allocatable :: prop_d(:,:,:), transf_d(:,:,:,:), ctf_d(:,:,:)
  complex(fp_kind),device,allocatable :: shift_arrayx_d(:,:), shift_arrayy_d(:,:)
  complex(fp_kind),device,allocatable :: shift_array_d(:,:), trans_d(:,:),lens_ctf(:,:,:)
  ...
  integer::jj,l,i_target,k,ii,i_df
    ...
    call load_save_add_grates(idum,projected_potential,nopiy,nopix,n_qep_grates&
                                                   &,n_slices,nt,nat_slice)
    ...
    if (any([on_the_fly,quick_shift,phase_ramp_shift])) then
	...
	if (pw_illum) then
        ...
        psi_initial = make_ctf(probe_initial_position,probe_df(1),probe_cutoff,&
	...
	do i = 1, n_slices
	    call make_propagator(nopiy,nopix,prop(:,:,i),prop_distance(i),Kz(ntilt),ss&
                                    &,ig1,ig2,claue(:,ntilt),ifactorx,ifactory)
	    prop(:,:,i) = prop(:,:,i) * bwl_mat
        if(.not.on_the_fly) then
		qep_grates(:,:,:,i) = exp(ci*pi*a0_slice(3,i)/Kz(ntilt)*projected_potential(:,:,:,i))
		do j=1,n_qep_grates
        qep_grates(:,:,j,i)= fft(nopiy,nopix,qep_grates(:,:,j,i),norm=.true.)*bwl_mat
        if(qep_mode.ne.3) call inplace_ifft(nopiy,nopix,qep_grates(:,:,j,i),norm=.true.)
		enddo
		endif
    enddo
    ...
    do i_qep_pass = 1, n_qep_passes
        ...
        do i_cell = 1,maxval(ncells)
	        do i_slice = 1, n_slices
                ...
                ! Phase grate
	            nran = floor(n_qep_grates*ran1(idum)) + 1
              call qep_multislice_iteration(psi,prop(:,:,i_slice),qep_grates(:,:,:,i_slice),&
                                            &nopiy,nopix,ifactory,ifactorx,idum,n_qep_grates,&
                                            &qep_mode,shift_arrayy,shift_arrayx)
          enddo ! End loop over slices
				...
				if (any(i_cell==ncells)) then
					...
					do i=1,imaging_ndf
						...
						call inplace_ifft(nopiy,nopix,psi_temp,norm=.true.)
...
900     format(1h+,  'QEP pass:', i4, '/', i4, ' Intensity: ', f8.3)
	...
	do i=1,nz
		...
		if(.not.output_thermal) then
			call binary_out_unwrap(nopiy, nopix, cbed(:,:,i), trim(filename)&
                                                &//'_DiffractionPattern')
			if(pw_illum) call binary_out(nopiy, nopix, total_intensity(:,:,i), &
                                     &trim(filename)//'_ExitSurface_Intensity')
		else
			call binary_out_unwrap(nopiy, nopix, cbed(:,:,i), trim(filename)&
                                                &//'_DiffractionPattern_Total')

			image = abs(fft(nopiy, nopix, psi_elastic(:,:,i),norm=.true.))**2
			call binary_out_unwrap(nopiy, nopix, image, trim(filename)//&
                                      &'_DiffractionPattern_Elastic')

			image = cbed(:,:,i) - image
			call binary_out_unwrap(nopiy, nopix, image, trim(filename)//&
                                          &'_DiffractionPattern_TDS')

			if(pw_illum) then
        call binary_out(nopiy, nopix, psi_elastic(:,:,i), trim(filename)//&
                                        &'_ExitSurface_Elastic')
        call binary_out(nopiy, nopix, total_intensity(:,:,i), trim(filename)//&
                                        &'_ExitSurface_Intensity_Total')

        total_intensity(:,:,i)=total_intensity(:,:,i)-abs(psi_elastic(:,:,i))**2
        call binary_out(nopiy, nopix, total_intensity(:,:,i),&
                         &trim(filename)//'_ExitSurface_Intensity_TDS')
			endif
		endif
		if(pw_illum) then
		do j=1,imaging_ndf
			! Elastic image
			call inplace_fft (nopiy, nopix, psi_elastic(:,:,i))
			psi_elastic(:,:,i) = psi_elastic(:,:,i) * ctf(:,:,j)/nopiy/nopix
			call inplace_ifft (nopiy, nopix, psi_elastic(:,:,i))
			image = abs(psi_elastic(:,:,i))**2

			if(imaging_ndf>1) then
                fnam_df = trim(adjustl(filename))//'_Defocus_'&
                          &//zero_padded_int(int(imaging_df(j)),lengthdf)//'_Ang'
            else
  ...
  if(double_channeling) then
    ...
    call output_TEM_result(output_prefix,tile_out_image(image,ifactory,ifactorx)&
    &,'energy_filtered_image',nopiy,nopix,manyz,imaging_ndf>1,manytilt,z=zarray(i)&
    &,lengthz=lengthz,lengthdf=lengthdf,tiltstring = &
    & tilt_description(claue(:,ntilt),ak1,ss,ig1,ig2),df = imaging_df(j))
  endif
#endif
		enddo
		endif
